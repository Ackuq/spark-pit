...
import org.apache.spark.sql.catalyst.util.DateTimeUtils.{
  convertSpecialDate,
  convertSpecialTimestamp,
  convertSpecialTimestampNTZ,
  getZoneId,
  stringToDate,
  stringToTimestamp,
...
/** The AstBuilder converts an ANTLR4 ParseTree into a catalyst Expression, LogicalPlan or
  * TableIdentifier.
  */
class CustomAstBuilder
    extends SqlBaseBaseVisitor[AnyRef]
    with SQLConfHelper
    with Logging {
  ...
  override def visitSingleTableSchema(
      ctx: SingleTableSchemaContext
  ): StructType = {
      ...
      val subQuery: LogicalPlan =
        plan(ctx.query).optionalMap(ctx.columnAliases)((columnAliases, plan) =>
          UnresolvedSubqueryColumnAliases(
            visitIdentifierList(columnAliases),
            plan
          )
        )
  ...
  /** Create a logical plan which allows for multiple inserts using one 'from' statement. These
    ...
    */
  override def visitMultiInsertQuery(
      ctx: MultiInsertQueryContext
  ): LogicalPlan = withOrigin(ctx) {
    val from = visitFromClause(ctx.fromClause)
  ...
  override def visitSingleInsertQuery(
    ...
    withInsertInto(
      ctx.insertInto(),
      plan(ctx.queryTerm).optionalMap(ctx.queryOrganization)(
        withQueryResultClauses
      )
    )
  }
  ...
  type InsertTableParams =
    (UnresolvedRelation, Seq[String], Map[String, Option[String]], Boolean)
  ...
  override def visitInsertOverwriteHiveDir(
      ctx: InsertOverwriteHiveDirContext
  ): InsertDirParams = withOrigin(ctx) {
    throw QueryParsingErrors.insertOverwriteDirectoryUnsupportedError(ctx)
  }
  ...
  override def visitDeleteFromTable(ctx: DeleteFromTableContext): LogicalPlan =
    withOrigin(ctx) {
      val table = createUnresolvedRelation(ctx.multipartIdentifier())
      val tableAlias =
        getTableAliasWithoutColumnAlias(ctx.tableAlias(), "DELETE")
      val aliasedTable =
        tableAlias.map(SubqueryAlias(_, table)).getOrElse(table)
  ...
  override def visitUpdateTable(ctx: UpdateTableContext): LogicalPlan =
    withOrigin(ctx) {
      val table = createUnresolvedRelation(ctx.multipartIdentifier())
      val tableAlias =
        getTableAliasWithoutColumnAlias(ctx.tableAlias(), "UPDATE")
      val aliasedTable =
        tableAlias.map(SubqueryAlias(_, table)).getOrElse(table)
  ...
  private def withAssignments(
      assignCtx: SqlBaseParser.AssignmentListContext
  ): Seq[Assignment] =
  ...
  override def visitMergeIntoTable(ctx: MergeIntoTableContext): LogicalPlan =
    withOrigin(ctx) {
      ...
      val targetTableAlias =
        getTableAliasWithoutColumnAlias(ctx.targetAlias, "MERGE")
      val aliasedTarget = targetTableAlias
        .map(SubqueryAlias(_, targetTable))
      ...
      val sourceTableAlias =
        getTableAliasWithoutColumnAlias(ctx.sourceAlias, "MERGE")
      val aliasedSource =
        sourceTableAlias
          .map(SubqueryAlias(_, sourceTableOrQuery))
          .getOrElse(sourceTableOrQuery)

      val mergeCondition = expression(ctx.mergeCondition)

      val matchedActions = ctx.matchedClause().asScala.map { clause =>
        {
      ...
      val notMatchedActions = ctx.notMatchedClause().asScala.map { clause =>
        {
            ...
            if (clause.notMatchedAction().ASTERISK() != null) {
              ...
              if (columns.size != values.size) {
                throw QueryParsingErrors
                  .insertedValueNumberNotMatchFieldNumberError(clause)
              }
              InsertAction(
                condition,
      ...
      if (
        matchedActionSize >= 2 && !matchedActions.init.forall(
          _.condition.nonEmpty
        )
      ) {
      ...
      if (
        notMatchedActionSize >= 2 && !notMatchedActions.init.forall(
          _.condition.nonEmpty
        )
      ) {
  ...
  override def visitPartitionSpec(
  ...
  ): Map[String, Option[String]] = withOrigin(ctx) {
    ...
    val parts = ctx.partitionVal.asScala.map { pVal =>
      ...
      val value = Option(pVal.constant).map(v =>
        visitStringConstant(v, legacyNullAsString)
      )
      name -> value
    }
    ...
    if (conf.caseSensitiveAnalysis) {
      ...
      checkDuplicateKeys(
        parts.map(kv => kv._1.toLowerCase(Locale.ROOT) -> kv._2).toSeq,
        ctx
      )
    }
    parts.toMap
  ...
  protected def visitNonOptionalPartitionSpec(
    ...
    visitPartitionSpec(ctx).map {
      case (key, None) =>
        throw QueryParsingErrors.emptyPartitionKeyError(key, ctx)
    ...
    expression(ctx) match {
      ...
      case other =>
        throw new IllegalArgumentException(
          s"Only literals are allowed in the " +
  ...
  private def withTransformQuerySpecification(
    ...
    if (transformClause.setQuantifier != null) {
      throw QueryParsingErrors.transformNotSupportQuantifierError(
    ...
    val (attributes, schemaLess) = if (transformClause.colTypeList != null) {
      ...
      val attrs = visitIdentifierSeq(transformClause.identifierSeq).map {
        name =>
  ...
  def visitCommonSelectQueryClausePlan(
    ...
    val withProject = if (aggregationClause == null && havingClause != null) {
      if (conf.getConf(SQLConf.LEGACY_HAVING_WITHOUT_GROUP_BY_AS_WHERE)) {
        ...
        withHavingClause(
          havingClause,
          Aggregate(Nil, namedExpressions, withFilter)
        )
      }
    } else if (aggregationClause != null) {
      val aggregate =
        withAggregationClause(aggregationClause, namedExpressions, withFilter)
  ...
  protected def getRowFormatDelimited(
      ctx: RowFormatDelimitedContext
  ): ScriptIOFormat = {
  ...
  override def visitFromClause(ctx: FromClauseContext): LogicalPlan =
    withOrigin(ctx) {
      val from = ctx.relation.asScala.foldLeft(null: LogicalPlan) {
        (left, relation) =>
          ...
          val join = right.optionalMap(left) { (left, right) =>
            if (relation.LATERAL != null) {
              if (!relation.relationPrimary.isInstanceOf[AliasedQueryContext]) {
                throw QueryParsingErrors.invalidLateralJoinRelationError(
      ...
      if (ctx.pivotClause() != null) {
        if (!ctx.lateralView.isEmpty) {
          throw QueryParsingErrors.lateralWithPivotInFromClauseNotAllowedError(
            ctx
          )
        }
    ...
    val windowMapView = baseWindowMap.mapValues {
      case WindowSpecReference(name) =>
        baseWindowMap.get(name) match {
          ...
          case None =>
            throw QueryParsingErrors.cannotResolveWindowReferenceError(
              name,
              ctx
            )
        }
  ...
  private def withAggregationClause(
    ...
    if (ctx.groupingExpressionsWithGroupingAnalytics.isEmpty) {
      ...
      if (ctx.GROUPING != null) {
        ...
        val groupingSets =
          ctx.groupingSet.asScala.map(
            _.expression.asScala.map(e => expression(e)).toSeq
          )
        Aggregate(
  ...
  override def visitGroupingAnalytics(
    ...
    if (groupingAnalytics.CUBE != null) {
      ...
      if (groupingSets.exists(_.isEmpty)) {
        throw QueryParsingErrors.invalidGroupingSetError(
          "CUBE",
          groupingAnalytics
        )
      }
      ...
      assert(
        groupingAnalytics.GROUPING != null && groupingAnalytics.SETS != null
      )
  ...
  /** Add [[UnresolvedHint]]s to a logical plan.
    */
  private def withHints(ctx: HintContext, query: LogicalPlan): LogicalPlan =
    withOrigin(ctx) {
      var plan = query
      ctx.hintStatements.asScala.reverse.foreach { stmt =>
        plan = UnresolvedHint(
          stmt.hintName.getText,
          stmt.parameters.asScala.map(expression).toSeq,
          plan
        )
      }
      plan
    ...
    val pivotColumn = if (ctx.pivotColumn.identifiers.size == 1) {
      ...
      CreateStruct(
        ctx.pivotColumn.identifiers.asScala
          .map(identifier => UnresolvedAttribute.quoted(identifier.getText))
          .toSeq
      )
    }
  ...
  private def withJoinRelations(
    ...
    ctx.joinRelation.asScala.foldLeft(base) { (left, join) =>
      withOrigin(join) {
        ...
        if (
          join.LATERAL != null && !join.right.isInstanceOf[AliasedQueryContext]
        ) {
        ...
        val (joinType, condition) = Option(join.joinCriteria) match {
          ...
          case None if join.NATURAL != null =>
            if (join.LATERAL != null) {
              throw QueryParsingErrors
                .lateralJoinWithNaturalJoinUnsupportedError(ctx)
            }
        ...
        if (join.LATERAL != null) {
          if (!Seq(Inner, Cross, LeftOuter).contains(joinType)) {
            throw QueryParsingErrors.unsupportedLateralJoinTypeError(
              ctx,
              joinType.toString
            )
          }
          LateralJoin(
            left,
            LateralSubquery(plan(join.right)),
            joinType,
            condition
          )
        } else {
      ...
      ctx.sampleMethod() match {
        ...
        case ctx: SampleByBytesContext =>
          ...
          if (bytesStr.matches("[0-9]+[bBkKmMgG]")) {
            ...
            throw QueryParsingErrors.invalidByteLengthLiteralError(
              bytesStr,
              ctx
            )
          }
        ...
        case ctx: SampleByBucketContext =>
          sample(
            ctx.numerator.getText.toDouble / ctx.denominator.getText.toDouble
          )
      }
    }
  ...
  override def visitTableValuedFunction(
      ctx: TableValuedFunctionContext
  ): LogicalPlan = withOrigin(ctx) {
    ...
    if (name.database.nonEmpty) {
      operationNotAllowed(
        s"table valued function cannot specify database name: $name",
        ctx
      )
    }
  ...
  private def expressionList(
      trees: java.util.List[ExpressionContext]
  ): Seq[Expression] = {
  ...
  override def visitLogicalBinary(ctx: LogicalBinaryContext): Expression =
    withOrigin(ctx) {
      ...
      def reduceToExpressionTree(low: Int, high: Int): Expression =
        high - low match {
          case 0 =>
            expressions(low)
          case 1 =>
    ...
    withOrigin(ctx) {
      ...
      ctx.kind.getType match {
        ...
        case SqlBaseParser.IN if ctx.query != null =>
          invertIfNotDefined(
            InSubquery(getValueExpressions(e), ListQuery(plan(ctx.query)))
          )
        case SqlBaseParser.IN =>
          invertIfNotDefined(
            In(e, ctx.expression.asScala.map(expression).toSeq)
          )
        case SqlBaseParser.LIKE =>
          Option(ctx.quantifier).map(_.getType) match {
            case Some(SqlBaseParser.ANY) | Some(SqlBaseParser.SOME) =>
              ...
              val expressions = expressionList(ctx.expression)
              if (
                expressions.forall(_.foldable) && expressions.forall(
                ...
                ctx.expression.asScala
                  .map(expression)
                  .map(p => invertIfNotDefined(new Like(e, p)))
                  .toSeq
                  .reduceLeft(Or)
              }
  ...
  override def visitFunctionCall(ctx: FunctionCallContext): Expression =
    withOrigin(ctx) {
      ...
      val ignoreNulls =
        Option(ctx.nullsOption)
          .map(_.getType == SqlBaseParser.IGNORE)
          .getOrElse(false)
  ...
  private def visitFunctionName(
    ...
    texts match {
      ...
      case other =>
        throw QueryParsingErrors.functionNameUnsupportedError(
          texts.mkString("."),
          ctx
        )
    }
  ...
  /** Get a function identifier consist by database (optional) and name.
    */
  protected def getFunctionIdentifier(
  ...
  override def visitLambda(ctx: LambdaContext): Expression = withOrigin(ctx) {
    val arguments = ctx.identifier().asScala.map { name =>
      UnresolvedNamedLambdaVariable(
        UnresolvedAttribute.quoted(name.getText).nameParts
      )
    }
  ...
  /** Create an [[UnresolvedExtractValue]] expression, this is used for subscript access to an array.
    */
  override def visitSubscript(ctx: SubscriptContext): Expression =
    withOrigin(ctx) {
    ...
    withOrigin(ctx) {
      ...
      def toLiteral[T](f: UTF8String => Option[T], t: DataType): Literal = {
        f(UTF8String.fromString(value)).map(Literal(_, t)).getOrElse {
          throw QueryParsingErrors
            .cannotParseValueTypeError(valueType, value, ctx)
        }
      }
      ...
      def constructTimestampLTZLiteral(value: String): Literal = {
        ...
        val specialTs =
          convertSpecialTimestamp(value, zoneId).map(Literal(_, TimestampType))
        specialTs.getOrElse(
      ...
      try {
        valueType match {
          case "DATE" =>
            val zoneId = getZoneId(conf.sessionLocalTimeZone)
            val specialDate =
              convertSpecialDate(value, zoneId).map(Literal(_, DateType))
                ...
                convertSpecialTimestampNTZ(
                  ...
                  .getOrElse {
                    val containsTimeZonePart =
                      DateTimeUtils
                        .parseTimestampString(UTF8String.fromString(value))
                        ._2
                        .isDefined
            ...
            val interval =
              try {
                ...
                case e: IllegalArgumentException =>
                  val ex =
                    QueryParsingErrors.cannotParseIntervalValueError(value, ctx)
  ...
  override def visitLegacyDecimalLiteral(
      ctx: LegacyDecimalLiteralContext
  ...
  /** Create a BigDecimal Literal expression.
    */
  override def visitBigDecimalLiteral(
      ctx: BigDecimalLiteralContext
  ): Literal = {
  ...
  private def createUnresolvedRelation(
      ctx: MultipartIdentifierContext
  ): UnresolvedRelation = withOrigin(ctx) {
  ...
  ): UnresolvedTable = withOrigin(ctx) {
    UnresolvedTable(
  ...
  ): UnresolvedTableOrView = withOrigin(ctx) {
  ...
  override def visitInterval(ctx: IntervalContext): Literal = withOrigin(ctx) {
    ...
    if (
      ctx.errorCapturingUnitToUnitInterval != null && !conf.legacyIntervalEnabled
    ) {
      ...
      val fromUnit =
        ctx.errorCapturingUnitToUnitInterval.body.from.getText
          .toLowerCase(Locale.ROOT)
      val toUnit = ctx.errorCapturingUnitToUnitInterval.body.to.getText
        .toLowerCase(Locale.ROOT)
      if (toUnit == "month") {
      ...
      ctx.errorCapturingMultiUnitsInterval != null && !conf.legacyIntervalEnabled
    ) {
      val units =
  ...
  /** Create a [[CalendarInterval]] object
    */
  protected def parseIntervalLiteral(ctx: IntervalContext): CalendarInterval =
    withOrigin(ctx) {
      if (ctx.errorCapturingMultiUnitsInterval != null) {
        ...
        if (innerCtx.error1 != null || innerCtx.error2 != null) {
          val errorCtx =
            if (innerCtx.error1 != null) innerCtx.error1 else innerCtx.error2
          throw QueryParsingErrors.moreThanOneFromToUnitInIntervalLiteralError(
            errorCtx
          )
        }
  ...
  /** Creates a [[CalendarInterval]] with multiple unit value pairs, e.g. 1 YEAR 2 DAYS.
    */
  override def visitMultiUnitsInterval(
      ctx: MultiUnitsIntervalContext
  ): CalendarInterval = {
    withOrigin(ctx) {
  ...
  /** Creates a [[CalendarInterval]] with from-to unit, e.g. '2-1' YEAR TO MONTH.
    */
  override def visitUnitToUnitInterval(
      ctx: UnitToUnitIntervalContext
  ): CalendarInterval = {
    withOrigin(ctx) {
      val value = Option(ctx.intervalValue.STRING)
        .map(string)
        .map { interval =>
        ...
        .getOrElse {
          throw QueryParsingErrors.invalidFromToUnitValueError(
            ctx.intervalValue
          )
        }
      try {
        ...
        (from, to) match {
          ...
          case ("day", "hour") | ("day", "minute") | ("day", "second") |
              ("hour", "minute") | ("hour", "second") | ("minute", "second") =>
          ...
          case _ =>
            throw QueryParsingErrors.fromToIntervalUnsupportedError(
              from,
              to,
              ctx
            )
  ...
  override def visitPrimitiveDataType(ctx: PrimitiveDataTypeContext): DataType =
    withOrigin(ctx) {
      ...
      (dataType, ctx.INTEGER_VALUE().asScala.toList) match {
        ...
        case (dt, params) =>
          val dtStr =
            if (params.nonEmpty) s"$dt(${params.mkString(",")})" else dt
          throw QueryParsingErrors.dataTypeUnsupportedError(dtStr, ctx)
      }
    }

  override def visitYearMonthIntervalDataType(
  ...
  override def visitDayTimeIntervalDataType(
    ...
    withOrigin(ctx) {
      ctx.complex.getType match {
        ...
        case SqlBaseParser.STRUCT =>
          StructType(
            Option(ctx.complexColTypeList).toSeq
              .flatMap(visitComplexColTypeList)
          )
      }
    }
  ...
  override def visitComplexColTypeList(
      ctx: ComplexColTypeListContext
  ): Seq[StructField] = withOrigin(ctx) {
  ...
  protected def visitLocationSpecList(
      ctx: java.util.List[LocationSpecContext]
  ): Option[String] = {
  ...
  protected def visitCommentSpecList(
      ctx: java.util.List[CommentSpecContext]
  ): Option[String] = {
          ...
          .map { orderedIdCtx =>
            Option(orderedIdCtx.ordering).map(_.getText).foreach { dir =>
              if (dir.toLowerCase(Locale.ROOT) != "asc") {
                operationNotAllowed(
                  s"Column ordering must be ASC, was '$dir'",
                  ctx
                )
              }
            }
  ...
  /** A table property value can be String, Integer, Boolean or Decimal. This function extracts
    ...
    */
  override def visitTablePropertyValue(
      value: TablePropertyValueContext
  ): String = {
    if (value == null) {
      null
  ...
  override def visitCreateTableHeader(
      ctx: CreateTableHeaderContext
  ): TableHeader = withOrigin(ctx) {
  ...
  override def visitPartitionTransform(
      ctx: PartitionTransformContext
  ): Transform = withOrigin(ctx) {
    ...
    ctx.transform match {
      case identityCtx: IdentityTransformContext =>
        IdentityTransform(
          FieldReference(typedVisit[Seq[String]](identityCtx.qualifiedName))
        )
        ...
        applyCtx.identifier.getText match {
          case "bucket" =>
            val numBuckets: Int = arguments.head match {
              ...
              case lit =>
                throw QueryParsingErrors.invalidBucketsNumberError(
                  lit.describe,
                  applyCtx
                )
            }
            ...
            val fields =
              arguments.tail.map(arg => getFieldReference(applyCtx, arg))
  ...
  /** Parse an argument to a transform. An argument may be a field reference (qualified name) or
    * a value literal.
    */
  override def visitTransformArgument(
      ctx: TransformArgumentContext
  ): V2Expression = {
    withOrigin(ctx) {
  ...
  /** Create a [[CreateNamespaceStatement]] command.
    ...
    * }}}
    */
  override def visitCreateNamespace(ctx: CreateNamespaceContext): LogicalPlan =
    withOrigin(ctx) {
      import SupportsNamespaces._
      ...
      if (!ctx.PROPERTIES.isEmpty && !ctx.DBPROPERTIES.isEmpty) {
        throw QueryParsingErrors.propertiesAndDbPropertiesBothSpecifiedError(
          ctx
        )
      }
  ...
  /** Create a [[DropNamespace]] command.
    ...
    * }}}
    */
  override def visitDropNamespace(ctx: DropNamespaceContext): LogicalPlan =
    withOrigin(ctx) {
      DropNamespace(
  ...
  /** Create an [[SetNamespaceProperties]] logical plan.
    ...
    * }}}
    */
  override def visitSetNamespaceProperties(
      ctx: SetNamespacePropertiesContext
  ): LogicalPlan = {
    withOrigin(ctx) {
      val properties = cleanNamespaceProperties(
  ...
  /** Create an [[SetNamespaceLocation]] logical plan.
    ...
    * }}}
    */
  override def visitSetNamespaceLocation(
      ctx: SetNamespaceLocationContext
  ): LogicalPlan = {
    withOrigin(ctx) {
  ...
  /** Create a [[ShowNamespaces]] command.
    */
  override def visitShowNamespaces(ctx: ShowNamespacesContext): LogicalPlan =
    withOrigin(ctx) {
  ...
  /** Create a [[DescribeNamespace]].
    ...
    * }}}
    */
  override def visitDescribeNamespace(
      ctx: DescribeNamespaceContext
  ): LogicalPlan =
    withOrigin(ctx) {
      DescribeNamespace(
        UnresolvedNamespace(
      ...
      (ctx.fileFormat, ctx.storageHandler) match {
        ...
        case (c: TableFileFormatContext, null) =>
          SerdeInfo(formatClasses =
            Some(FormatClasses(string(c.inFmt), string(c.outFmt)))
          )
  ...
  protected def validateRowFormatFileFormat(
    ...
    (rowFormatCtx, createFileFormatCtx.fileFormat) match {
      ...
      case (
            rfDelimited: RowFormatDelimitedContext,
            ffGeneric: GenericFileFormatContext
          ) =>
        ffGeneric.identifier.getText.toLowerCase(Locale.ROOT) match {
          case "textfile" => // OK
  ...
  protected def validateRowFormatFileFormat(
    ...
    if (rowFormatCtx.size == 1 && createFileFormatCtx.size == 1) {
      validateRowFormatFileFormat(
        rowFormatCtx.head,
        createFileFormatCtx.head,
        parentCtx
      )
    }
  }

  override def visitCreateTableClauses(
    ...
    val (partTransforms, partCols) =
      Option(ctx.partitioning)
        .map(visitPartitionFieldList)
        .getOrElse((Nil, Nil))
    ...
    val (cleanedOptions, newLocation) =
      cleanTableOptions(ctx, options, location)
    val comment = visitCommentSpecList(ctx.commentSpec())
    val serdeInfo =
      getSerdeInfo(
        ctx.rowFormat.asScala.toSeq,
        ctx.createFileFormat.asScala.toSeq,
        ctx
      )
    (
      partTransforms,
  ...
  protected def getSerdeInfo(
    ...
    (fileFormatSerdeInfo ++ rowFormatSerdeInfo).reduceLeftOption((l, r) =>
      l.merge(r)
    )
  }
  ...
  override def visitCreateTable(ctx: CreateTableContext): LogicalPlan =
    withOrigin(ctx) {
      ...
      val columns =
        Option(ctx.colTypeList()).map(visitColTypeList).getOrElse(Nil)
      val provider =
        Option(ctx.tableProvider).map(_.multipartIdentifier.getText)
      val (
        partTransforms,
        partCols,
        bucketSpec,
      ...
      if (provider.isDefined && serdeInfo.isDefined) {
        operationNotAllowed(
          s"CREATE TABLE ... USING ... ${serdeInfo.get.describe}",
          ctx
        )
      }
  ...
  override def visitReplaceTable(ctx: ReplaceTableContext): LogicalPlan =
    withOrigin(ctx) {
      val (table, temp, ifNotExists, external) =
        visitReplaceTableHeader(ctx.replaceTableHeader)
      val orCreate = ctx.replaceTableHeader().CREATE() != null

      if (temp) {
        val action = if (orCreate) "CREATE OR REPLACE" else "REPLACE"
        operationNotAllowed(
          s"$action TEMPORARY TABLE ..., use $action TEMPORARY VIEW instead.",
          ctx
        )
      }
      ...
      if (ifNotExists) {
        operationNotAllowed(
          "REPLACE ... IF NOT EXISTS, use CREATE IF NOT EXISTS instead",
          ctx
        )
      }
  ...
  override def visitShowCurrentNamespace(
      ctx: ShowCurrentNamespaceContext
  ): LogicalPlan = withOrigin(ctx) {
  ...
  /** Create a [[ShowViews]] command.
    */
  override def visitShowViews(ctx: ShowViewsContext): LogicalPlan =
    withOrigin(ctx) {
      val multiPart =
        Option(ctx.multipartIdentifier).map(visitMultipartIdentifier)
      ShowViews(
  ...
  override def visitQualifiedColTypeWithPosition(
    ...
    QualifiedColType(
      path =
        if (name.length > 1) Some(UnresolvedFieldName(name.init)) else None,
      colName = name.last,
      dataType = typedVisit[DataType](ctx.dataType),
    ...
    withOrigin(ctx) {
      ...
      AddColumns(
        createUnresolvedTable(
          ctx.multipartIdentifier,
          s"ALTER TABLE ... ADD $colToken"
        ),
        ctx.columns.qualifiedColTypeWithPosition.asScala
  ...
  override def visitAlterTableAlterColumn(
    ...
    val position = if (action.colPosition != null) {
      Some(
        UnresolvedFieldPosition(typedVisit[ColumnPosition](action.colPosition))
      )
    } else {
      None
    }
  ...
  /** Parse a [[AlterTableAlterColumn]] command. This is Hive SQL syntax.
    ...
    * }}}
    */
  override def visitHiveChangeColumn(
      ctx: HiveChangeColumnContext
  ): LogicalPlan = withOrigin(ctx) {
    if (ctx.partitionSpec != null) {
      operationNotAllowed(
        "ALTER TABLE table PARTITION partition_spec CHANGE COLUMN",
        ctx
      )
    }
  ...
  override def visitUnsetTableProperties(
      ctx: UnsetTablePropertiesContext
  ): LogicalPlan = withOrigin(ctx) {
    val properties = visitPropertyKeys(ctx.tablePropertyList)
    val cleanedProperties =
      cleanTableProperties(ctx, properties.map(_ -> "").toMap).keys.toSeq
  ...
  /** Create an [[SetTableLocation]] command.
    ...
    * }}}
    */
  override def visitSetTableLocation(
  ...
  /** Create a [[DescribeColumn]] or [[DescribeRelation]] commands.
    */
  override def visitDescribeRelation(
      ctx: DescribeRelationContext
  ): LogicalPlan = withOrigin(ctx) {
    val isExtended = ctx.EXTENDED != null || ctx.FORMATTED != null
    val relation =
      createUnresolvedTableOrView(ctx.multipartIdentifier(), "DESCRIBE TABLE")
    if (ctx.describeColName != null) {
      ...
      val partitionSpec = if (ctx.partitionSpec != null) {
        ...
        visitPartitionSpec(ctx.partitionSpec).map {
          case (key, Some(value)) => key -> value
          case (key, _) =>
            throw QueryParsingErrors.incompletePartitionSpecificationError(
              key,
              ctx
            )
        }
  ...
  override def visitAnalyze(ctx: AnalyzeContext): LogicalPlan =
    withOrigin(ctx) {
      ...
      if (
        ...
        throw QueryParsingErrors.computeStatisticsNotExpectedError(
          ctx.identifier()
        )
      }
  ...
  override def visitRefreshTable(ctx: RefreshTableContext): LogicalPlan =
    withOrigin(ctx) {
      RefreshTable(
        createUnresolvedTableOrView(ctx.multipartIdentifier(), "REFRESH TABLE")
      )
    }
      ...
      val tableWithNamespace =
        if (namespace.isDefined && table.multipartIdentifier.length == 1) {
          CurrentOrigin.withOrigin(table.origin) {
            table.copy(multipartIdentifier =
              namespace.get ++ table.multipartIdentifier
            )
          }
        } else {
          table
  ...
  override def visitRenameTablePartition(
      ctx: RenameTablePartitionContext
  ): LogicalPlan = withOrigin(ctx) {
    RenamePartitions(
      createUnresolvedTable(
        ctx.multipartIdentifier,
  ...
  override def visitDropTablePartitions(
      ctx: DropTablePartitionsContext
  ): LogicalPlan = withOrigin(ctx) {
    if (ctx.VIEW != null) {
  ...
  /** Create an [[SetTableSerDeProperties]]
    ...
    * }}}
    */
  override def visitSetTableSerDe(ctx: SetTableSerDeContext): LogicalPlan =
    withOrigin(ctx) {
  ...
  override def visitCreateView(ctx: CreateViewContext): LogicalPlan =
    withOrigin(ctx) {
      ...
      val userSpecifiedColumns =
        Option(ctx.identifierCommentList).toSeq.flatMap { icl =>
          icl.identifierComment.asScala.map { ic =>
            ic.identifier.getText -> Option(ic.commentSpec())
              .map(visitCommentSpec)
          }
        }

      val properties = ctx.tablePropertyList.asScala.headOption
        .map(visitPropertyKeyValues)
        .getOrElse(Map.empty)
  ...
  /** Create a plan for a DESCRIBE FUNCTION statement.
    */
  override def visitDescribeFunction(
      ctx: DescribeFunctionContext
  ): LogicalPlan = withOrigin(ctx) {
    import ctx._
    val functionName =
    ...
    withOrigin(ctx) {
      val resources = ctx.resource.asScala.map { resource =>
        ...
        resourceType match {
          ...
          case other =>
            operationNotAllowed(
              s"CREATE FUNCTION with resource type '$resourceType'",
              ctx
            )
        }
      }
  ...
  override def visitCommentNamespace(
      ctx: CommentNamespaceContext
  ): LogicalPlan = withOrigin(ctx) {
  ...
  override def visitCommentTable(ctx: CommentTableContext): LogicalPlan =
    withOrigin(ctx) {
      ...
      CommentOnTable(
        createUnresolvedTable(ctx.multipartIdentifier, "COMMENT ON TABLE"),
        comment
      )
    }

  private def alterViewTypeMismatchHint: Option[String] = Some(
    "Please use ALTER TABLE instead."
  )

  private def alterTableTypeMismatchHint: Option[String] = Some(
